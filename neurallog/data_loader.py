...
def gpt2_encoder(s, no_wordpiece=0):
    ...
    try:
        inputs = gpt2_tokenizer(
            s, return_tensors="pt", max_length=512, padding=True, truncation=True
        )
...
def load_HDFS(
    ...
    for i, line in enumerate(logs):
        ...
        if (i + 1) % 1000 == 0:
            print(
                f"\rProcessing {(i+1)/len(logs)*100:.2f}% - {len(E)} unique messages",
                end="",
            )
...
def load_supercomputers(
    ...
    x_te, y_te = [], []
    for i in range(
        n_train, len(logs) - windows_size, step_size if step_size > 0 else windows_size
    ):
        seq = []
        label = 0
        ...
        if len(x_te) % 1000 == 0:
            print(
                f"\rProcessing test data: {(i-n_train)/(len(logs)-n_train)*100:.2f}%",
                end="",
            )
